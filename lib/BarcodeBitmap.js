...
export class BarcodeBitmap extends Bitmap {
  ...
  static makeBarcode(format, digits, shortHeight) {
    if (!/^[0-9]{6,12}[?0-9]$/.test(digits)) {
      throw new Error(
        'Value must consist only of digits, optionally\n' +
        'followed by "?" (a question mark). The total\n' +
        'length must be between 7 and 13 characters.'
      );
    }
    ...
    switch (format) {
      ...
      case 'dwim': {
        switch (digits.length) {
          ...
          default: {
            throw new Error('Value must have 7, 8, 12, or 13 digits.');
          }
        }
        break;
      }
